...
sag_keys <- do.call("rbind", lapply(sag_years, function(x) findAssessmentKey(stock = NULL,
                                                                             year = x,
...
sag_summary <- bind_rows(
 found_stocks %>%
    ...
    mutate(SummaryTable = purrr::map(AssessmentKey, function(x) getSummaryTable(x)[[1]])) %>%
   # as.tibble %>%
    tidyr::unnest(SummaryTable),
  found_stocks %>%
    filter(is.na(AssessmentKey))
) %>%
  select(Year,
...
sag_ref_pts <- bind_rows(
  found_stocks %>%
    filter(!is.na(AssessmentKey)) %>%
    mutate(RefTable = purrr::map(AssessmentKey, function(x) getFishStockReferencePoints(x)[[1]])) %>%
    tidyr::unnest(RefTable),
  found_stocks %>%
    filter(is.na(AssessmentKey))
) %>%
  select(StockCode,
...
# suppressWarnings(lapply(grep("Greater|Baltic",
#                              unique(pie_table_count$EcoRegion),
...
stock_trends_fun <- function(EcoGuild,
  ...
  clicks <- sag_complete_summary %>%
    ...
    if(length(adj_names) <= 10) {
      values <- tableau_color_pal('tableau10')(length(adj_names))
...
filename <- paste0("~/git/ices-dk/fisheryO/output/figure07_Discards_", fileName, "-static.png")
